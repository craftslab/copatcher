...
func getAPTImageName(manifest *types.UpdateManifest) string {
	...
	if manifest.Metadata.OS.Type == debianOS {
...
func (dm *dpkgManager) probeDPKGStatus(ctx context.Context, toolImage string) error {
	...
	mkFolders := busyBoxApplied.File(llb.Mkdir(resultsPath, fileMode, llb.WithParents(true)))
...
func (dm *dpkgManager) unpackAndMergeUpdates(ctx context.Context, updates types.UpdatePackages, toolImage string) (*llb.State, error) {
	...
	mkFolders := downloaded.File(llb.Mkdir(resultsPath, fileMode, llb.WithParents(true))).File(llb.Mkdir(dpkgStatusFolder, fileMode, llb.WithParents(true)))
...
func validateDebianPackageVersions(updates types.UpdatePackages, cmp VersionComparer, resultsPath string, ignoreErrors bool) ([]string, error) {
	...
	for _, update := range updates {
		...
		if cmp.LessThan(version, update.UpdatedVersion) {
			errorPkgs = append(errorPkgs, update.Name)
			err = fmt.Errorf("downloaded package %s version %s lower than required %s for update", update.Name, version, update.UpdatedVersion)
